...
public class Main extends JavaPlugin{
	...
	ConcurrentHashMap<String, Location> arrows = new ConcurrentHashMap<String, Location>();
	ConcurrentHashMap<String, Location> oldarrows = new ConcurrentHashMap<String, Location>();
	
	List<SurvivalVoter> voters = new LinkedList<SurvivalVoter>();
	
	ConcurrentHashMap<String, Scoreboard> scoreboards = new ConcurrentHashMap<String, Scoreboard>();
            	...
            	if (name.equals("unfriend")){
    		        scheduler.scheduleSyncDelayedTask(plugin, new Runnable() {
    		            ...
    		            public void run(){
    						...
    						menu.setName(plugin.utils.replace(plugin.getConfig().getString("menu.friend.name")).replaceAll("<player>", friendMenuViewing.get(event.getPlayer().getName())));
		...
		menuArrow = new Menu(utils.replace(getConfig().getString("menu.arrow.name")), 9, new Menu.OptionClickEventHandler() {
            ...
            public void onOptionClick(final Menu.OptionClickEvent event) {
            	...
            	if (name.equals("toggle")){
					...
					List<ArrowPoint> list = plugin.db.getArrowPoints(event.getPlayer().getName());
					int size = list.size();
					if (event.getPlayer().hasPermission("survival.max")){
						if (size > 53){
							event.getPlayer().sendMessage(plugin.utils.replace(plugin.getConfig().getString("menu.arrow.setpoint.too-many")).replaceAll("<number>", "54"));
							return;
						}
					} else {
						if (size > 9){
							event.getPlayer().sendMessage(plugin.utils.replace(plugin.getConfig().getString("menu.arrow.setpoint.too-many")).replaceAll("<number>", "10"));
							return;
						}
					}
	...
	public void showFriendList(final Player p){
            ...
            public void onOptionClick(final Menu.OptionClickEvent event) {
		        ...
		        scheduler.scheduleSyncDelayedTask(plugin, new Runnable() {
		            ...
		            public void run(){
						...
						menu.setName(plugin.utils.replace(plugin.getConfig().getString("menu.friend.name")).replaceAll("<player>", event.getName()));
		            	menu.open(p);
		            }
		        }, 2L);
                event.setWillClose(true);
		...
		for (String s : db.getFriends(chunks.get(0))){
			...
			if (Bukkit.getOfflinePlayer(s).isOnline()) lore.add(ChatColor.GREEN + getConfig().getString("online"));
	...
	public void showBannedList(final Player p){
            ...
            public void onOptionClick(final Menu.OptionClickEvent event) {
		        ...
		        scheduler.scheduleSyncDelayedTask(plugin, new Runnable() {
		            ...
		            public void run(){
						...
						menu.setName(plugin.utils.replace(plugin.getConfig().getString("menu.unban.name")).replaceAll("<player>", event.getName()));
		            	menu.open(p);
		            }
		        }, 2L);
                event.setWillClose(true);
		...
		List<Chunk> chunks = db.getChunks(p.getName());
		int i = 0;
		for (String s : db.getBannedPlayers(chunks.get(0))){
			...
			meta.setLore(lore);
			item.setItemMeta(meta);
			menuBannedList.setOption(i, item, s);
			i++;
		}
	...
	public void showPointsList(final Player p){
            ...
            public void onOptionClick(final Menu.OptionClickEvent event) {
		        scheduler.scheduleSyncDelayedTask(plugin, new Runnable() {
		            ...
		            public void run(){
						...
						String dn = plugin.utils.replace(plugin.getConfig().getString("menu.point.name")).replaceAll("<point>", event.getName());
						menu.setName(dn);
		            	menu.open(event.getPlayer());
		            	
		            	for (ArrowPoint ap : dbarrowpoints){
		            		if (ap.name.equals(event.getName()) && ap.player.equals(event.getPlayer().getName())){
		...
		List<ArrowPoint> list = db.getArrowPoints(p.getName());
		int i = 0;
